...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # hopufield_layer
        num_prototype = 1000
        self.lookup = nn.Linear(embed_dim, num_prototype ,bias=False)   
        self.beta = 1./sqrt(embed_dim)
        
        self.content = nn.Sequential(
            nn.Linear(num_prototype, embed_dim,bias=False),
    ...
    def forward_features(self, z, x, event_z, event_x,
        ...
        for i, blk in enumerate(self.blocks):
            x, global_index_t, global_index_s, removed_index_s, attn = \
                blk(x, global_index_t, global_index_s, mask_x, ce_template_mask, ce_keep_rate, i)

            if self.ce_loc is not None and i in self.ce_loc:
                removed_indexes_s.append(removed_index_s)
        
        # The last hopufield layer       
        x = torch.softmax(self.lookup(x)*self.beta, dim=-1)
        x = self.content(x)
