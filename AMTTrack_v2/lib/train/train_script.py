...
from lib.train.actors import AMTTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "amttrack":
        net = build_amttrack(cfg)
    else:
    ...
    if settings.script_name == "amttrack":
        ...
        actor = AMTTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
