...
class Felt(BaseVideoDataset):
    ...
    def _get_event_voxel_sequence_path(self, seq_id):  # get events_voxel_dir_path
        ...
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel0")
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel1")
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel2")
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel3")
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel4")
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel5")
        # return os.path.join(self.root, self.sequence_list[seq_id], self.sequence_list[seq_id]+ "_voxel6")
    ...
    def _get_event_voxel(self, seq_path, frame_id):  # get a events voxel
        ...
        for f_id in frame_id:
            ...
            if os.path.getsize(event_frame_file) == 0:  # 如果文件大小为0，表示文件为空或不存在有效数据
                ...
                # event_features = np.zeros(4096, 11)  # voxel.mat文件为空时，创建一个形状为 (4096, 19)的全零数组作为事件特征，表示没有事件发生or数据无效
                # event_features = np.zeros(4096, 35)  # voxel.mat文件为空时，创建一个形状为 (4096, 19)的全零数组作为事件特征，表示没有事件发生or数据无效
                ...
                if np.isnan(event_features).any():  # 处理NaN值
                    ...
                    # event_features = np.zeros(4096, 11)                     
                    # event_features = np.zeros(4096, 35)                     
