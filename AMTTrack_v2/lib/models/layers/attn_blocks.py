...
from lib.models.layers.attn import Attention, Attention_asymmetric
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, asymmetric_flag=False):
        super().__init__()
        self.asymmetric_flag = asymmetric_flag
        self.norm1 = norm_layer(dim)
        if self.asymmetric_flag:
            self.attn = Attention_asymmetric(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        else:
            self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, mask=None, lens_z=64, lens_x=256):
        if self.asymmetric_flag:
            x_attn, attn = self.attn(self.norm1(x), mask, True, lens_z=lens_z, lens_x=lens_x)
        else:
            x_attn, attn = self.attn(self.norm1(x), mask, True)
